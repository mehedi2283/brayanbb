...
export function SettingsView() {
  ...
  useEffect(() => {
    fetch('/api/agency-key')
      .then(res => res.json())
      .then(data => {
        if (data.agencyApiKey) {
          setAgencyKey(data.agencyApiKey);
        }
        setLoading(false);
      })
      .catch(err => {
        console.error('Failed to load agency key', err);
        setLoading(false);
      });
  }, []);
  ...
  const handleSave = async () => {
    ...
    setMessage('');
    try {
      const res = await fetch('/api/agency-key', {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        setMessage('Failed to save Agency API Key.');
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden max-w-2xl mx-auto mt-10">
      <div className="p-6 border-b border-slate-100">
        ...
        <p className="text-sm text-slate-500 mt-1">Configure your GoHighLevel Agency API settings.</p>
      </div>
      <div className="p-6 space-y-6">
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-2">Agency API Key</label>
          <input
            type="password"
            value={agencyKey}
          ...
          <p className="text-xs text-slate-500 mt-2">
            This key is used to fetch the list of sub-accounts from GoHighLevel.
          </p>
        </div>
